  ...
  return (
    <div className="screen usage-screen">
      <section className="page-heading">
        <div><Eyebrow>Usage · measured production economics</Eyebrow><h1>Know every frame’s cost.</h1><p>Live session timing and verified outputs—never decorative estimates disguised as billing.</p></div>
      ...
      {chartRuns.length ? (
        <div className="usage-layout">
          <section className="panel usage-chart-panel">
            ...
            <footer className="chart-note"><Leaf size={16} /><span>ImageForge ranks available approved GPUs by measured batch cost when you explicitly press Start GPU. It never waits on or starts a preferred GPU automatically.</span></footer>
          </section>

          <aside className="panel live-cost-panel">
            <header className="panel-heading panel-heading--compact"><div><Eyebrow>Current attachment</Eyebrow><h2>{state.pod.gpu ?? 'GPU offline'}</h2></div><Zap size={20} /></header>
            {state.pod.gpu ? (
              <>
                ...
                <dl className="live-cost-details">
                  <div><dt>Pod ID</dt><dd>{state.pod.podId ?? 'discovering'}</dd></div>
                  <div><dt>Attachment</dt><dd>Disposable compute</dd></div>
      ...
      <section className="panel usage-table-panel">
        <header className="panel-heading"><div><Eyebrow>Run ledger</Eyebrow><h2>Recent measured batches</h2></div><span>{chartRuns.length} records</span></header>
        <div className="usage-table-wrap">
          <table className="usage-table">
            <thead><tr><th>Batch</th><th>GPU</th><th>Verified</th><th>Failed</th><th>Elapsed</th><th>Cost</th><th>Cost / image</th></tr></thead>
            <tbody>
